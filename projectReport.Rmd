...
dev.off()
```
From the three models, we can see that the best parameters are
- Random Forest: predictors = 4
- Radial Basis Function: Cost = 3.0, sigma = 0.03
- Support Vector Machine: Cost = 3.0

Though, the Support Vector Machine is much less accurate than the other two, by quite the margin.
...
```{r}
# Validation set
pred_wCOMB <- predict(wCOMB, newdata = wVal)
confusionMatrix(pred_wCOMB, wVal$classe)
```
...
plot(wCOMB, main = "Tuning Parameters for GBM Combined Model")
dev.off()
```
