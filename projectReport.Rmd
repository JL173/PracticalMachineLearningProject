...
grid <- expand.grid(c(0.01, 0.03, 0.1, 0.3, 1.0, 3.0))
...
pred_RBF <- predict(wRBF, newdata = wTest)
pred_wSVM <- predict(wSVM, newdata = wTest)
...
acc(pred_wCOMB, wTest$classe)
```

```{r}
# testing accuracy on the validation set to check Combined accuracy

val_wRF <- predict(wRF, newdata = wVal)
val_wRBF <- predict(wRBF, newdata = wVal)
val_wSVM <- predict(wSVM, newdata = wVal)
...
# percentage of correct classifications using helper function
print(c(
  acc(val_wRF, wVal$classe), acc(val_wRBF, wVal$classe),
  acc(val_wSVM, wVal$classe), acc(val_wCOMB, wVal$classe)
))
