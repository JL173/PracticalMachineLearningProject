...
Our models did not train on the validation set, and so we can use the results on this set to estimate the 'out of sample error'. Our combined model gave us an accuracy of `0.9915` on the validation set. This is only a slight improvement of the Random Forest Model, `0.9913`, but since Random Forests can be prone to overfitting, combining with the Radial Basis Function and Support Vector Machine should help alleviate any bias in that sense, and it appears to have done so.